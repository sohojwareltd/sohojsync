 ...
 * Clean and modern navigation sidebar with collapse feature
 */
const AdminSidebar = () => {
  const { user } = useAuth();
  ...
  const [screenTime, setScreenTime] = useState({ formatted: '—', seconds: 0 });
  ...
  useEffect(() => {
    ...
    const fetchScreenTime = async () => {
      try {
        ...
        console.log('[AdminSidebar] Fetched screen time data:', data);
        setScreenTime({
          formatted: data?.formatted || formatSeconds(data?.screen_time_seconds || 0),
          seconds: data?.screen_time_seconds || 0,
        });
      } catch (error) {
    ...
    const handleVisibilityChange = () => {
      if (!document.hidden) {
        console.log('[AdminSidebar] Page visibility changed, refetching screen time');
        fetchScreenTime();
      }
    };
  ...
  const settingsItem = { path: '/admin/settings', icon: SettingsIcon, label: 'Settings' };
      ...
      <div className="h-16 flex items-center justify-between px-6">
        {!collapsed && (
          <div className="flex items-center gap-2.5">
            <div className="w-8 h-8 rounded-lg flex items-center justify-center" style={{background: `linear-gradient(135deg, ${PRIMARY_COLOR} 0%, rgb(109, 106, 177) 100%)`}}>
        ...
        {collapsed && (
          <div className="w-8 h-8 rounded-lg flex items-center justify-center mx-auto" style={{background: `linear-gradient(135deg, ${PRIMARY_COLOR} 0%, rgb(109, 106, 177) 100%)`}}>
            <svg className="w-4 h-4 text-white" fill="currentColor" viewBox="0 0 20 20">
              <path d="M10.894 2.553a1 1 0 00-1.788 0l-7 14a1 1 0 001.169 1.409l5-1.429A1 1 0 009 15.571V11a1 1 0 112 0v4.571a1 1 0 00.725.962l5 1.428a1 1 0 001.17-1.408l-7-14z"/>
            </svg>
          </div>
        ...
        <div className="space-y-1">
          {navItems.map((item) => (
            ...
            >
              {({ isActive }) => (
                <>
                  <div className={`w-5 h-5 flex items-center justify-center flex-shrink-0 ${isActive ? 'text-purple-600' : 'text-gray-400 group-hover:text-gray-600'}`}>
                    <item.icon className="w-5 h-5" />
                  </div>
                  {!collapsed && (
                    <span className={`text-[14px] font-medium ${isActive ? 'text-purple-600' : 'text-gray-600 group-hover:text-gray-900'}`}>
                      {item.label}
                    </span>
                  )}
                </>
        ...
        <div className="bg-gradient-to-br from-purple-50 to-purple-100 rounded-2xl p-4">
          ...
          <div className="flex items-center gap-3 mb-4">
            <div 
              ...
              style={{background: `linear-gradient(135deg, ${PRIMARY_COLOR} 0%, rgb(109, 106, 177) 100%)`}}
            >
              {user?.name?.[0]?.toUpperCase() || 'A'}
            </div>
            <div className="flex-1 min-w-0">
              <p className="text-sm font-bold text-gray-900 truncate">{user?.name || 'Admin'}</p>
              <p className="text-xs text-gray-600 capitalize truncate">{user?.role?.replace('_', ' ') || 'Administrator'}</p>
            </div>
          </div>
          ...
          <div className="bg-white/60 rounded-xl p-3 mb-3">
            ...
            <p className="text-lg font-bold" style={{color: PRIMARY_COLOR}}>{screenTime.formatted}</p>
          </div>
