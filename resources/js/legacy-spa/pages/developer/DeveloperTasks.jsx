...
/**
 * Developer Tasks Page - View only assigned tasks
 */
const DeveloperTasks = () => {
    ...
    const fetchTasks = async () => {
        try {
            // API now handles proper role-based filtering
        ...
        <div className="p-4 space-y-4">
            ...
            <div className="bg-white rounded-lg p-3 shadow-sm border" style={{ borderColor: '#e5e7eb' }}>
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
                    <div>
                        <h1 className="text-base font-semibold text-gray-800">My Tasks</h1>
                        <p className="text-sm text-gray-600 mt-0.5">Tasks assigned to you</p>
                    </div>
                </div>
            </div>
                    ...
                    <div className="flex items-center gap-2">
                        {['all', 'open', 'in_progress', 'done'].map((status) => (
                            <button
                                ...
                                className={`px-3 py-2 rounded-lg text-sm font-medium transition-all ${filter === status ? 'text-white shadow-sm' : 'text-gray-600 hover:bg-gray-100'
                                    }`}
                            ...
                            <div className="flex items-start justify-between gap-3">
                                <div className="flex-1 min-w-0">
                                    ...
                                    <div className="flex items-center gap-3 mt-2">
                                        <span className={`px-2 py-0.5 rounded text-xs font-medium ${task.status === 'done' ? 'bg-green-100 text-green-700' :
